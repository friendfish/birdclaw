...
- [x] Implement `runDigestArchivePreSyncEffect` with these exported contracts:
  ...
  Resolve the transport through `resolveMentionsDataSource()` so the existing environment-over-config precedence is preserved: `bird`/`xurl`/`auto` use that live transport, while `birdclaw` records local/skipped steps. For You always calls `syncHomeTimelineEffect` with `mode:"bird"` and `following:false`. Following uses the resolved live transport. Mentions use the resolved transport; omit `startTime` for Bird because that transport does not support it. After the mention operation, collect local mention IDs for the requested period window and call `syncMentionThreadsEffect` with the concrete transport actually used by mentions, including an `auto` fallback to Bird. Catch each operation independently and record `degraded` plus the error instead of failing the whole effect. Treat partial thread results or per-thread failures as degraded; configured local skips remain visible per step but do not degrade an otherwise successful batch.
